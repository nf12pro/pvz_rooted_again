...
#region Variables
var plant_scene: PackedScene = preload("res://scenes/plants/world_1_basic/frost_peashooter.tscn")
var overlay_scene: PackedScene = preload("res://scenes/plants/world_1_basic/plant_seeds/overlays/frost_peashooter_overlay.tscn")
...
func _on_input_event(_viewport, event, _shape_idx):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if Global.sun_value >= cost:
			is_selected = true
			Global.frost_pea_is_selected = true
		...
		for item in results:
			...
			if clicked_node is Area2D and clicked_node.has_method("snap_plant"):
				...
				if clicked_node.has_plant:
					...
					if existing_plant and "is_icepea" in existing_plant:
						print("Upgrading to Twin Sunflower!")
						...
						is_selected = false
						Global.frost_pea_is_selected = false
