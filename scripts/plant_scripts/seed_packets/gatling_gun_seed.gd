...
func _on_input_event(_viewport, event, _shape_idx):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if Global.sun_value >= cost:
			is_selected = true
			Global.gatling_gun_is_selected = true
	...
	if is_selected and event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		for item in results:
			...
			if clicked_node is Area2D and clicked_node.has_method("snap_plant"):
				# Only upgrade if the tile already has a Repeater
				if clicked_node.has_plant:
					print("Check")
					var existing_plant = clicked_node.current_plant
					if existing_plant and "is_repeater" in existing_plant:
						print("Upgrading Repeater to Gatling Gun!")
						...
						is_selected = false
						Global.gatling_gun_is_selected = false
		...
		# If clicked tile has no Repeater, just cancel selection
		is_selected = false
		Global.gatling_gun_is_selected = false
