...
func _ready() -> void:
	...
	if Global.upgrade_seed_1 == "Gatling Gun":
		gatling_gun_seed.position = Vector2(720, 55)
		get_tree().current_scene.call_deferred("add_child", gatling_gun_seed)
	elif Global.upgrade_seed_1 == "Twin Sunflower":
		twin_seed.position = Vector2(720, 55)
		get_tree().current_scene.call_deferred("add_child", twin_seed)
	elif Global.upgrade_seed_1 == "Plasma Wood":
		plasma_wood_seed.position = Vector2(720, 55)
		get_tree().current_scene.call_deferred("add_child", plasma_wood_seed)
	elif Global.upgrade_seed_1 == "Tall Nut":
		tallnut_seed.position = Vector2(720, 55)
		get_tree().current_scene.call_deferred("add_child", tallnut_seed)
	elif Global.upgrade_seed_1 == "Plasma Pea":
		plasma_pea_seed.position = Vector2(720, 55)
		get_tree().current_scene.call_deferred("add_child", plasma_pea_seed)
	elif Global.upgrade_seed_1 == "Frost Pea":
		frost_pea_seed.position = Vector2(720, 55)
