...
#region Variables
var plant_scene: PackedScene = preload("res://scenes/plants/world_1_basic/tallnut.tscn")
var overlay_scene: PackedScene = preload("res://scenes/plants/world_1_basic/plant_seeds/overlays/tallnut_overlay.tscn")
		...
		for item in results:
			...
			if clicked_node is Area2D and clicked_node.has_method("snap_plant"):
				...
				if clicked_node.has_plant:
					...
					if existing_plant and "is_wallnut" in existing_plant:
						print("Upgrading to Tall Nut!")
